...
const StartStreaming = () => {
  ...
  const sendMessage = () => {
    if (messageInput.trim()) {
      const newMessage = { user: firebaseUser?.displayName || "Guest", text: messageInput };
      ...
      {!isStreaming ? (
        ...
        <>
          <div className="video-controls">
            <video ref={videoRef} autoPlay muted playsInline className="stream-video" />
            <Space wrap style={{ marginTop: "1rem" }}>
              <Button icon={isMuted ? <AudioMutedOutlined /> : <AudioOutlined />} onClick={toggleMute}>
                {isMuted ? "Unmute" : "Mute"}
              </Button>
              <Button icon={isCameraOn ? <VideoCameraOutlined /> : <VideoCameraAddOutlined />} onClick={toggleCamera}>
                {isCameraOn ? "Camera Off" : "Camera On"}
              </Button>
              <Button icon={<DesktopOutlined />} onClick={startScreenShare}>
                Share Screen
              </Button>
              ...
              <Button danger icon={<LogoutOutlined />} onClick={leaveStream}>
                Leave Stream
              </Button>
            </Space>
          </div>
