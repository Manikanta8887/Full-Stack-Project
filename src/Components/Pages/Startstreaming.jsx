...
//     });
//     socket.on("update-streams", (streams) => {
//       const myStream = streams.find((s) => s.streamId === streamId);
//       if (myStream) {
//         setViewerCount(myStream.viewers || 0);
//       }
//     });
...
//         socket.emit("rejoin-stream", { streamId });
//         message.success("Rejoined the stream after refresh.");
//       } catch (err) {
//         console.error("Rejoin error:", err);
//         message.error("Failed to rejoin stream after refresh.");
...
//     if (isStreaming && !peerConnectionRef.current && streamId) {
//       dispatch(setIsStreaming(true));
//       dispatch(setStreamTitle(streamTitle));
//       rejoinStream();
//     }
//   }, [isStreaming, streamTitle, streamId, dispatch]);
...
//       setMessageInput("");
//     }
//   }, [messageInput, firebaseUser, streamId]);
...
import React, { useEffect, useRef, useState, useCallback } from "react";
import { useSelector, useDispatch } from "react-redux";
import { setIsStreaming, setStreamTitle, setStreamId } from "../Redux/streamSlice.js";
import { Button, Card, Space, message, Typography, Input, Row, Col, List } from "antd";
import {
  VideoCameraOutlined,
  StopOutlined,
...
const StartStreaming = () => {
  ...
  const startStream = async () => {
    ...
    try {
      ...
      setStreamId(newStreamId);
      localStorage.setItem("streamId", newStreamId);
      dispatch(setStreamId(newStreamId));
