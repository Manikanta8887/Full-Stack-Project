...
const StartStreaming = () => {
  ...
  // Socket event listeners for chat and ICE candidates
  useEffect(() => {
  ...
  // Scroll chat into view automatically
  useEffect(() => {
    chatEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [messages]);

  // Rejoin stream logic to handle a refresh without ending the stream immediately
  useEffect(() => {
    const rejoinStream = async () => {
      try {
        // Clean up existing stream tracks if any
        if (videoRef.current?.srcObject) {
          videoRef.current.srcObject.getTracks().forEach((track) => track.stop());
        }
        const mediaStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
        console.log("STREAMER: Reacquired media stream for rejoin");
        videoRef.current.srcObject = mediaStream;
        setStream(mediaStream);
        setOriginalVideoTrack(mediaStream.getVideoTracks()[0]);
        peerConnectionRef.current = new RTCPeerConnection(servers);
    ...
    if (isStreaming && !peerConnectionRef.current && firebaseUser) {
      dispatch(setIsStreaming(true));
      dispatch(setStreamTitle(streamTitle));
      rejoinStream();
    }
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      if (isStreaming) {
        ...
        e.returnValue = "";
        rejoinTimeoutRef.current = setTimeout(() => {
          stopStream(true); // Silent stop if not rejoined within 5 seconds
        }, 5000);
      }
    };
    window.addEventListener("beforeunload", handleBeforeUnload);
  ...
  const toggleScreenShare = async () => {
    try {
      if (!navigator.mediaDevices.getDisplayMedia) {
        message.error("Screen sharing is not supported in this browser.");
        return;
      }
      const screenStream = await navigator.mediaDevices.getDisplayMedia({ video: true });
      const screenTrack = screenStream.getVideoTracks()[0];
  ...
  const startStream = async () => {
    ...
    try {
      setIsLoading(true);
      const mediaStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
      // Immediately attach the media stream to the video element
      videoRef.current.srcObject = mediaStream;
      setStream(mediaStream);
      setOriginalVideoTrack(mediaStream.getVideoTracks()[0]);
      dispatch(setIsStreaming(true));
