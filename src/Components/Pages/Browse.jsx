...
const VideoCard = ({ vid }) => {
  ...
  return (
    <Card hoverable className="browse-video-card-wrapper">
      <div className="browse-video-thumbnail" onClick={() => setPlaying(!playing)}>
        {playing ? (
          <video
            src={vid.url}
            className="browse-video-player"
            controls
            autoPlay
            muted
          />
          ...
          <>
            <img
              ...
              className="browse-video-poster"
            />
            <div className="browse-play-icon">
              <PlayCircleOutlined style={{ fontSize: 32, color: "#fff" }} />
            </div>
          </>
        )}
      </div>
      <div className="browse-video-title">
        <b>{vid.title || "Untitled Video"}</b>
      </div>
    </Card>
  );
};
