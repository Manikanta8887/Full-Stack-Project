...
const Browse = () => {
  ...
  useEffect(() => {
    // Request the current lists
    socket.emit("get-streams");

    socket.on("stream-list", ({ liveStreams, pastStreams }) => {
      setLiveStreams(liveStreams);
      setPastStreams(pastStreams);
    });

    // Whenever any stream starts/stops, server will broadcast the full liveStreams array
    socket.on("update-streams", (streams) => {
      setLiveStreams(streams);
    });

    socket.on("start-stream", (newStream) => {
      // show a toast but liveStreams will update via update-streams
      message.success(`"${newStream.streamTitle}" is now live!`);
    });
    ...
    <div className="browse-container">
      ...
      <Row gutter={[16, 16]}>
        {pastStreams.length === 0 ? (
          ...
          pastStreams.map((stream) => (
            <Col xs={24} sm={12} md={8} lg={6} key={stream.streamId}>
              <Card
                hoverable
                cover={
                  <img
